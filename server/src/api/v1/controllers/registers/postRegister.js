...
import { insertRegister } from "../../models/registers/insertRegister.js";
import { handleError } from "../../utils/utils.js";
import { transporter } from "../../../../../config/email/correoPrueba.js";
...
const addRegister = async (req, res) => {
  ...
  try {
    ...
    // Envío de correo automático con remitente definido ✅
    await transporter.sendMail({
      from: `"INACEX Matricúlate" <${process.env.EMAIL_USER}>`, // 👈 evita MAIL FROM: <>
      to: correo,
      subject: 'Confirma tu registro en INACEX',
      html: `
        <div style="font-family: 'Roboto', sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="https://inacexcem.cl/v1/confirmar-correo?correo=${correo}" 
               style="background-color: #007d3c; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold;">
               ✅ Confirmar correo
            </a>
          </div>
          ...
          <p style="font-size: 13px; color: gray;">Este es un mensaje automático, por favor no respondas.</p>
          <img src="https://inacexcem.cl/imgCursos/caex-cat.png" alt="INACEX Banner" />
        </div>
      `,
    });
    ...
    console.error("💥 Error desde DB o envío de correo:", error);
    const errorFound = handleError(error.code) || [
      { status: 500, message: "Error interno del servidor al insertar datos o enviar correo" },
    ];
    return res
      .status(errorFound[0]?.status || 500)
      .json({ error: errorFound[0]?.message || "Error interno del servidor" });
  }
};
